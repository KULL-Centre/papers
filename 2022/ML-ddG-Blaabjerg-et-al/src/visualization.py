...
def learning_curve_cavity(acc_val_list, acc_train_list, loss_train_list):
    ...
    lns1 = ax1.plot(epochs, loss_train_list, label="Train: Cross-entropy loss", color="blue")
    ...
    fig.savefig(f"{os.path.dirname(os.getcwd())}/output/cavity_models/cavity_learning_curve.pdf", bbox_inches='tight')
    plt.close()

def learning_curve_ds(pearson_r_list, val_loss_list, train_loss_list, model_idx, run_name):
    fig, ax1 = plt.subplots()
    epochs = np.arange(len(pearson_r_list))+1
    lns1 = ax1.plot(epochs, pearson_r_list, label="Val: Pearson $\\rho$", color="orange")
    ...
    lns3 = ax2.plot(epochs, train_loss_list, label="Train: MAE$_{F}$ loss", color="blue")
    ax2.set_ylabel("MAE$_{F}$ loss")
    ax2.yaxis.set_major_formatter(FormatStrFormatter('%.3f'))
    lns = lns1+lns2+lns3
    labs = [l.get_label() for l in lns]
    ax2.legend(lns, labs, loc="center right")
    fig.savefig(f"{os.path.dirname(os.getcwd())}/output/ds_models/ds_model_{model_idx}/learning_curve_{run_name}.pdf", bbox_inches='tight')
    plt.close()

def learning_curve_ds_with_errorbars(run_name):
    ...
    lns1 = ax1.errorbar(epochs, np.mean(model_pearson_all,axis=0), yerr=np.std(model_pearson_all, axis=0), label="Val: Pearson $\\rho$", color="orange")
    ax1.set_ylabel("Pearson $\\rho$")
    ax1.set_xlabel("Training epochs")
    ax1.set_ylim(0.70, 0.80)
    ...
    ax1.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
    ax2 = ax1.twinx()
    lns2 = ax2.errorbar(epochs, np.mean(model_val_loss_all,axis=0), yerr=np.std(model_val_loss_all,axis=0), label="Val: MAE$_{F}$ loss", color="green")
    lns3 = ax2.errorbar(epochs, np.mean(model_train_loss_all,axis=0), yerr=np.std(model_train_loss_all,axis=0), label="Train: MAE$_{F}$ loss", color="blue")
    ax2.set_ylabel("MAE$_{F}$ loss")
    ax2.yaxis.set_major_formatter(FormatStrFormatter('%.3f'))
    ...
    labs = [l.get_label() for l in lns]
    ax2.legend(lns, labs, loc="center right")
    fig.savefig(f"{os.path.dirname(os.getcwd())}/output/ds_models/ds_learning_curve_with_errorbars_{run_name}.pdf", bbox_inches='tight')
    plt.close()
...
def hist_plot(df, dataset_key):
    ...
    fig.savefig(f"{os.path.dirname(os.getcwd())}/output/{dataset_key}/total_pred_hist.pdf", bbox_inches='tight')
    plt.close()
    ...
    for pdbid in prot_names:
        ...
        fig.savefig(f"{os.path.dirname(os.getcwd())}/output/{dataset_key}/{pdbid}_pred_hist.pdf", bbox_inches='tight')
        plt.close()
...
def make_scatter(x,y, x_vs_y, dataset_key, pdb):
    ...
    if dataset_key == "Rosetta":
        xylim = ((-1,7),(-1,7))
    elif dataset_key=="Protein_G" or dataset_key=="ProTherm" or "ProTherm_with_Protein_G":
        xylim = ((-2,7),(-2,7))
    ...
    if pdb == None:
        ...
        plot_name = pdb
    fig.savefig(f"{os.path.dirname(os.getcwd())}/output/{dataset_key}/{plot_name}_{x_vs_y}_scatter.pdf", bbox_inches='tight')
    plt.close()
...
def make_scatter_combined(df, dataset_key, x_vs_y, prot_names):
    ...
    if dataset_key == "ProTherm_with_Protein_G":
        ...
        for i, pdbid in enumerate(prot_names):
            ...
            # Add data points
            ax = fig.add_subplot(1,1,1)
            ax.scatter(x, y, s=2, c=colors[i], label=f"{prot_names[i]}, Pearson $\\rho$: {pearson_r:.2f}")
        ...
        # Save
        plot_name = "combined"
        fig.savefig(f"{os.path.dirname(os.getcwd())}/output/{dataset_key}/{dataset_key}_{plot_name}_{x_vs_y}_scatter.pdf", bbox_inches='tight')
        plt.close()

    if dataset_key == "VAMP":

        # Map PDB ids to protein names
        name_dict = {"5BON": "NUDT15 (PDB: 5BON)",
                     "2H11": "TPMT (PDB: 2H11)",
                     "1D5R": "PTEN (PDB: 1D5R)",
                    }
        ...
        for i, pdbid in enumerate(prot_names):
            ...
            # Add data points
            ax.scatter(x, y_ml, s=2, c="red", label=f"RaSP - Pearson $\\rho$: {pearson_r_ml:.2f}")
            ax.scatter(x, y_rosetta, s=2, c="blue", label=f"Rosetta - Pearson $\\rho$: {pearson_r_rosetta:.2f}")
            ...
            # Save
            plot_name = "combined"
            fig.savefig(f"{os.path.dirname(os.getcwd())}/output/{dataset_key}/{dataset_key}_{plot_name}_{x_vs_y}_scatter_{pdbid}.pdf", bbox_inches='tight')
            plt.close()
...
def make_heatmap(dataset_key, mat, aa_list, description):
    ...
    fig.savefig(f"{os.path.dirname(os.getcwd())}/output/{dataset_key}/heatmap_{description}.pdf", bbox_inches='tight')
    plt.close()

def loss_analysis(df, dataset_key):
    # Order aa by chemistry (aromatic, hydrophobic, hydroxylic, polar, negatively charged)
    aa_list = ["W","Y","F","L","M","I","V","G","A","S","T","C","N","H","Q","K","R","D","E","P"]
    ...
    for i, res_wt in enumerate(aa_list):
        for j, res_mt in enumerate(aa_list):
            mat_loss_ml_rosetta[i,j] = df["mae_ml_rosetta"][(df['wt']==res_wt)&(df['mt']==res_mt)].mean()
            mat_var_rosetta[i,j] = df["score_rosetta_fermi"][(df['wt']==res_wt)&(df['mt']==res_mt)].var()
            if len(df[(df['wt']==res_wt) & (df['mt']==res_mt)])>0:
                mat_highloss_count_rosetta[i,j] = len(df_highloss_rosetta[(df_highloss_rosetta['wt']==res_wt) \
                                                  & (df_highloss_rosetta['mt']==res_mt)]) \
                                                  / len(df[(df['wt']==res_wt) & (df['mt']==res_mt)])
            if dataset_key != "Rosetta" and dataset_key != "Xstal_vs_AF2":         
                mat_loss_ml_exp[i,j] = df["mae_ml_exp"][(df['wt']==res_wt)&(df['mt']==res_mt)].mean()
                mat_var_exp[i,j] = df["score_exp_fermi"][(df['wt']==res_wt)&(df['mt']==res_mt)].var()
                if len(df[(df['wt']==res_wt)&(df['mt']==res_mt)])>0:
                    mat_highloss_count_exp[i,j] = len(df_highloss_exp[(df_highloss_exp['wt']==res_wt) \
    ...
    make_heatmap(dataset_key, mat_highloss_count_rosetta, aa_list, "highloss_count_rosetta")
    if dataset_key != "Rosetta" and dataset_key != "Xstal_vs_AF2":
...
def homology_plot(df, dataset_key):
    ...
    x_dict = {"1LZ1": [100, 98, 78, 62, 47, 33],
    ...
    for i, pdb in enumerate(pdb_list):
        ...
        for i, binid in enumerate(bin_list):
            ...
            if len(df_bin_mean) != 0:
                corr_ml.append(stats.pearsonr(df_bin_mean["score_ml"],df_bin_mean["score_exp"])[0])
                corr_rosetta.append(stats.pearsonr(df_bin_mean["score_rosetta"],df_bin_mean["score_exp"])[0])

        # Make scatter plot
        l1 = ax.plot(x, corr_rosetta, label="Rosetta", color="blue", marker=".")
        l2 = ax.plot(x, corr_ml, label="RaSP", color="orange", marker=".")
        anchored_text = AnchoredText(name_dict[pdb], loc="upper right",prop={'size': 8})
        ax.add_artist(anchored_text)
    ...
    fig.savefig(f"{os.path.dirname(os.getcwd())}/output/{dataset_key}/homology_plot.pdf", bbox_inches='tight')    
...
def plot_gnomad_clinvar():
    # Load data
    df = pd.read_csv(f"{os.path.dirname(os.getcwd())}/data/test/GnomAD_ClinVar/df.csv")
    ...
    # Data arrays
    benign_arr = np.sort(np.array(df['RaSP;score_ml'].values[np.where(df['clinvar;Clinvar_signifiance'].values == 'benign')]))
    pathogenic_arr = np.sort(np.array(df['RaSP;score_ml'].values[np.where(df['clinvar;Clinvar_signifiance'].values == 'pathogenic')]))

    gnomAD0_arr = np.sort(np.array(df['RaSP;score_ml'].values[np.where(df['gnomAD_category'].values == 'gnomAD AF > 1e-2')]))
    gnomAD1_arr = np.sort(np.array(df['RaSP;score_ml'].values[np.where(df['gnomAD_category'].values == '1e-2 > gnomAD AF > 1e-4')]))
    gnomAD2_arr = np.sort(np.array(df['RaSP;score_ml'].values[np.where(df['gnomAD_category'].values == 'gnomAD AF < 1e-4')]))
    ...
    pt.RainCloud(x = x_back, y = y_RaSP, data = df,
                order = ['Back'], orient='h',
                palette = sns.color_palette('colorblind')[7:], bw=.1, alpha = .65, ax=axs[0])

    # ClinVar 'benign' classified RaSP distribution plotted as a raincloud
    pt.RainCloud(x = x_clinvar, y = y_RaSP, data = df,
                order = ['benign'], orient='h',
                palette = sns.color_palette('colorblind')[0:], bw=.1, alpha = .65, ax=axs[1])

    cat_benign_median = np.nanmedian(df['RaSP;score_ml'].values[np.where(df['clinvar;Clinvar_signifiance'].values == 'benign')])
    i_benign,j_benign = intvl_95(benign_arr)

    axs[1].annotate('Median: '+str(np.around(cat_benign_median,decimals=2))+' '+'['+str(i_benign)+';'+str(j_benign)+']',
                    (-.5,.25), fontsize=11, color=sns.color_palette('colorblind')[0])

    # ClinVar 'pathogenic' classified RaSP distribution plotted as a raincloud
    pt.RainCloud(x = x_clinvar, y = y_RaSP, data = df,
                order = ['pathogenic'], orient='h',
                palette = sns.color_palette('colorblind')[1:], bw=.1, alpha = .65, ax=axs[2])

    cat_patho_median = np.nanmedian(df['RaSP;score_ml'].values[np.where(df['clinvar;Clinvar_signifiance'].values == 'pathogenic')])
    i_patho,j_patho = intvl_95(pathogenic_arr)

    axs[2].annotate('Median: '+str(np.around(cat_patho_median,decimals=2))+' '+'['+str(i_patho)+';'+str(j_patho)+']',
                    (-.5,.25), fontsize=11, color=sns.color_palette('colorblind')[1])

    # RaSP distribution for gnomAD AF_tot > 1e-2 plotted as a raincloud
    pt.RainCloud(x = x_gnomad, y = y_RaSP, data = df,
                order = ['gnomAD AF > 1e-2'], palette = sns.color_palette('colorblind')[2:],
                orient='h', bw=.1, alpha = .65, ax=axs[3])

    cat_0_median = np.nanmedian(df['RaSP;score_ml'].values[np.where(df['gnomAD_category'].values == 'gnomAD AF > 1e-2')])
    i_cat0,j_cat0 = intvl_95(gnomAD0_arr)

    axs[3].annotate('Median: '+str(np.around(cat_0_median,decimals=2))+' '+'['+str(i_cat0)+';'+str(j_cat0)+']',
                    (-.5,.25), fontsize=11, color=sns.color_palette('colorblind')[2])

    # RaSP distribution for 1e-2 > gnomAD AF_tot > 1e-4 plotted as a raincloud
    pt.RainCloud(x = x_gnomad, y = y_RaSP, data = df,
                order = ['1e-2 > gnomAD AF > 1e-4'],
                palette = sns.color_palette('colorblind')[3:],
                orient='h', bw=.1, alpha = .65, ax=axs[4])

    cat_1_median = np.nanmedian(df['RaSP;score_ml'].values[np.where(df['gnomAD_category'].values == '1e-2 > gnomAD AF > 1e-4')])
    i_cat1,j_cat1 = intvl_95(gnomAD1_arr)

    axs[4].annotate('Median: '+str(np.around(cat_1_median,decimals=2))+' '+'['+str(i_cat1)+';'+str(j_cat1)+']',
                    (-.5,.25), fontsize=11, color=sns.color_palette('colorblind')[3])

    # RaSP distribution for gnomAD AF_tot < 1e-4 plotted as a raincloud
    pt.RainCloud(x = x_gnomad, y = y_RaSP, data = df,
                order = ['gnomAD AF < 1e-4'], palette = sns.color_palette('colorblind')[4:],
                orient='h', bw=.1, alpha = .65, ax=axs[5])

    cat_2_median = np.nanmedian(df['RaSP;score_ml'].values[np.where(df['gnomAD_category'].values == 'gnomAD AF < 1e-4')])
    i_cat2,j_cat2 = intvl_95(gnomAD2_arr)

    axs[5].annotate('Median: '+str(np.around(cat_2_median,decimals=2))+' '+'['+str(i_cat2)+';'+str(j_cat2)+']',
                    (-.5,.25), fontsize=11, color=sns.color_palette('colorblind')[4])
    ...
    fig.align_labels()
    fig.savefig(f"{os.path.dirname(os.getcwd())}/output/GnomAD_ClinVar/rasp_gnomad_clinvar.pdf", bbox_inches='tight')
