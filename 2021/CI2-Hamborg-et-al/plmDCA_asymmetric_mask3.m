...
% Modified by Daniele Granata and Pierre Barrat-Charlaix 10/2017
...
function plmDCA_asymmetric_mask3(fastafile_q,outputfile_scores, outputfile_parameters, maskfile,reweighting_threshold,nr_of_cores,lambda,weights_file)
...
% A note on progTol: In our experiments on PFAM-families, a progTol of 1e-3 gave identical true-positive rates to 1e-9 (default), but with moderately shorter running time. Differences in the scores between progTol 1e-3 and 1e-9 showed up in the 3rd-4th decimal or so (which tends to matter little when ranking them). We here set 1e-7 to be on the safe side, but this can be raised to gain speed. If, however, one wishes to use the scores for some different application, or extract and use the parameters {h,J} directly, we recommend the default progTol 1e-9.
    ...
    %addpath('/storage1/dgranata/WEIGT/DECIMATION/plmDCA_decimation-master/sources/plmDCA_mask/functions')
    %addpath('/storage1/dgranata/WEIGT/DECIMATION/plmDCA_decimation-master/sources/plmDCA_mask/3rd_party_code/minFunc')

    addpath('/storage1/shared/software/SEQ_MODEL_DESIGN/plmDCA_mask/functions')
    addpath('/storage1/shared/software/SEQ_MODEL_DESIGN/plmDCA_mask/3rd_party_code/minFunc')
...
%denotes average.
     ...
     for i=1:(N-1)
         for j=(i+1):N
             ...
             output=[output;[i,j,NORMS(i,j)-CORRNORMS(i,j)/(N-1)]];
         end
     end
