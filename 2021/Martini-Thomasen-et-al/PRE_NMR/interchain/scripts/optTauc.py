...
def calcChi2(prot,model,chains):
    chi2 = 0
    for label in prot.labels:
        y = np.loadtxt(prot.path+model+'/res{:s}-{:d}.dat'.format(chains,label))[:,2]
...
def loadInitPREs(name,prot,model,chains):
    value = {}
    error = {}
    resnums = np.arange(1,len(prot.fasta)+1)
    for label in prot.labels:
        value[label], error[label] = np.loadtxt(prot.path+model+'/res{:s}-{:d}.dat'.format(chains,label),usecols=(2,3),unpack=True)
    v = pd.DataFrame(value,index=resnums)
    v.rename_axis('residue', inplace=True)
    v.rename_axis('label', axis='columns',inplace=True)
    e = pd.DataFrame(error,index=resnums)
...
def optTauC(prot,model,chi2_results,rs_results,Nreplicas,tau_c_range,chains):
    chi2list = []
    for tc in tau_c_range:
        ...
        for label in prot.labels:
            ...
            for run in range(1,Nreplicas+1):
                x,y = np.loadtxt(prot.path+model+'/run{:d}/calcPREs/res{:s}-{:d}.dat'.format(run,chains,label),usecols=(0,1),unpack=True)
                measured_resnums = np.where(~np.isnan(y))[0] 
                data = pd.read_pickle(prot.path+model+'/run{:d}/calcPREs/res{:s}-{:d}.pkl'.format(run,chains,label), compression='gzip')
    ...
    chi2_results.to_pickle('chi2_'+args.name+'_'+chains+'_tc.pkl')
    rs_results.to_pickle('rs_'+args.name+'_'+chains+'_tc.pkl')
    ...
    for label in prot.labels:
        ...
        for run in range(1,Nreplicas+1):
            ...
            np.savetxt(prot.path+model+'/run{:d}/calcPREs/res{:s}-{:d}.dat'.format(run,chains,label),np.c_[x,i_ratio,gamma_2_av])
            all_i_ratio.append(i_ratio)
            all_gamma_2.append(gamma_2_av)
        np.savetxt(prot.path+model+'/res{:s}-{:d}.dat'.format(chains,label),np.c_[x,np.mean(all_i_ratio,axis=0),np.mean(all_gamma_2,axis=0),np.std(all_gamma_2,axis=0)/np.sqrt(Nreplicas)])
 
    return tc_min, calcChi2(prot,model,chains)

for name in proteins.index:
    proteins.at[name,'expPREs'] = loadExpPREs(name,proteins.loc[name])

tau_c_range = np.arange(args.tcmin,10.05,1)
...
for chains in ['AB','BA']:
    for i in [1.00,1.06,1.08]:
        model = '{:.2f}'.format(i)
        tau_c, chi2_pre = optTauC(proteins.loc[args.name],model,chi2_results,rs_results,Nreplicas,tau_c_range,chains)
        PREs[model] = {'chi2':chi2_pre, 'tau_c':tau_c, 'expPREs':proteins.at[args.name,'expPREs'], 'calcPREs':loadInitPREs(args.name,proteins.loc[args.name],model,chains)}
        print(args.name,model,tau_c)
    pd.DataFrame(PREs).to_pickle('{:s}_{:s}_PREs.pkl'.format(args.name,chains))
