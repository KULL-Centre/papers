...
def optTauC(prot,model,chi2_results,rs_results,Nreplicas,tau_c_range):
    ...
    for tc in tau_c_range:
        ...
        for label in prot.labels:
            ...
            for run in range(1,Nreplicas+1):
                x,y = np.loadtxt(prot.path+model+'/run{:d}/calcPREs/res-{:d}.dat'.format(run,label),usecols=(0,1),unpack=True)
                measured_resnums = np.where(~np.isnan(y))[0] 
                data = pd.read_pickle(prot.path+model+'/run{:d}/calcPREs/res-{:d}.pkl'.format(run,label), compression='gzip')
        ...
        chi2_results.loc[tc,model] = 0.1 * chi2 / len(prot.labels)
    ...
    for label in prot.labels:
        ...
        for run in range(1,Nreplicas+1):
            ...
            np.savetxt(prot.path+model+'/run{:d}/calcPREs/res-{:d}.dat'.format(run,label),np.c_[x,i_ratio,gamma_2_av])
