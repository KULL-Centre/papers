...
def calcContactMap(traj,df,prot,name,contact_map,energies,lambdamap,sigmamap,lj_eps):
    ...
    ah = lambda x,sig,l : np.where(x<=np.power(2.,1./6)*sig,lj(x,sig)+lj_eps*(l-1),l*lj(x,sig))
...
def trajCM(df,proteins,name,ff,run):
    ...
    energies = np.empty(0)
    #for traj in :
    len_chunk = t.n_frames//4
    print(t.n_frames,len_chunk*4,lambdamap.shape,sigmamap.shape,lj_eps)
    for k in range(4):
        b = len_chunk*k
        e = b+len_chunk
    ...
    contact_map = contact_map.reshape(len(prot.fasta),len(prot.fasta)) / 4
