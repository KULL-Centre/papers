...
# # May crash
# print(global_fit)
# print(summary(global_fit))

################################################################################
# Gradient example
################################################################################
# f_expr = expression((A+B*exp(x))/(1+exp(x)))
# f=function(x,A,B) {eval(f_expr, list(x=x,A=A,B=B))}

# # Hand differentiation
# df=function(x,A,B) {e=exp(x); B*e/(1+e) - (A*e+B*e*e)/((1+e)*(1+e))}

# # Analytic differentiation
# Df_expr=D(f_expr,"x")
# Df=function(x,A,B) {eval(Df_expr, list(x=x,A=A,B=B))}

# # plot
# x=seq(-10,10,0.01)
# plot(x, f(x,1,0), type="l", ylim=c(-.5,1))
# lines(x, df(x,1,0), col=2, lwd=2)
# lines(x, Df(x,1,0), col=3)
